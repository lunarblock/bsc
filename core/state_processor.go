...
func (p *ParallelStateProcessor) executionSlotLoop(schedulerInfo *ParallelSchedulingInfo) {
	for {
		...
		case txReq := <-schedulerInfo.executionQueue:
			...
			// create new slotDB
			slotDB := state.NewSlotDB(schedulerInfo.mainStateDB, consensus.SystemAddress, p.mergedTxIndex, txReq.keepSystem)
	...
	for {
		...
		case txResult := <-schedulerInfo.validationQueue:
			...
			if txResult.err != nil {
				...
				log.Info("need system address redo", "txIdx", txReq.txIndex)
				hasConflict = true
				systemAddrConflict = true
			} else {
				// check all finalizedDb from current slot's
				for _, changeList := range schedulerInfo.mergedChangeList {
					if changeList.TxIndex <= slotDB.BaseTxIndex() {
						continue
					}
					if p.hasStateConflict(slotDB, changeList) {
						log.Info("has state conflict", "txIdx", txReq.txIndex)
						hasConflict = true
						break
					}
				}
...
func (p *ParallelStateProcessor) finalizationSlotLoop(schedulerInfo *ParallelSchedulingInfo) {
	for {
		...
		case txResult := <-schedulerInfo.finalizationQueue:
			...
			changeList := schedulerInfo.mainStateDB.MergeSlotDB(txResult.slotDB, txResult.receipt, txIndex)
			schedulerInfo.mergedChangeList = append(schedulerInfo.mergedChangeList, changeList)
