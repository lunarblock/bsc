...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config) (*state.StateDB, types.Receipts, []*types.Log, uint64, error) {
	...
	if txNum > 0 {
		log.Info("Process", "block num", block.Number(), "txNum", txNum)
	}
			...
			if writeKeys, ok := writes[readAddr]; ok {
				...
				for writeKey := range writeKeys {
					...
					if _, ok := readKeys[writeKey]; ok {
						log.Debug("hasStateConflict state conflict", "addr", readAddr, "key", writeKey)
						return true
					}
				}
			}
	...
	if len(balanceReads) != 0 && len(balanceWrite) != 0 {
		for readAddr := range balanceReads {
			...
			if _, ok := balanceWrite[readAddr]; ok {
				...
				log.Debug("hasStateConflict balance conflict", "addr", readAddr)
				return true
			}
		}
	}
...
func (p *StateProcessor) execInParallelSlot(slotIndex int, txReq *ParallelTxRequest) *ParallelTxResult {
	...
	log.Debug("execInParallelSlot enter", "Slot", slotIndex, "txIndex", txIndex, "slotDB.baseTxIndex", slotDB.BaseTxIndex())
	...
	log.Debug("execInParallelSlot Stage Execution done", "Slot", slotIndex, "txIndex", txIndex, "slotDB.baseTxIndex", slotDB.BaseTxIndex())
	...
	log.Debug("execInParallelSlot Tx Stage Execution done, do conflict check", "Slot", slotIndex, "txIndex", txIndex)
	if slotDB.SystemAddressRedo() {
		hasConflict = true
		systemAddrConflict = true
	} else {
		for index := 0; index < state.ParallelExecNum; index++ {
			...
			for _, mergedInfo := range p.slotState[index].mergedTxInfo {
				...
				if p.hasStateConflict(slotDB, mergedInfo) {
					log.Debug("execInParallelSlot Stage Execution conflict", "Slot", slotIndex,
	...
	// ok, time to do finalize, this stage should not be parallel
	...
	if result.Err != nil {
		...
		log.Debug("execInParallelSlot TX reverted?", "Slot", slotIndex, "txIndex", txIndex, "result.Err", result.Err)
	}
...
func (p *StateProcessor) runSlotLoop(slotIndex int) {
	...
	for {
		log.Debug("parallel slot waiting", "slotIndex:", slotIndex)
		// wait for new TxReq
...
func (p *StateProcessor) InitParallelOnce() {
	...
	p.paraTxResultChan = make(chan *ParallelTxResult, state.ParallelExecNum) // fixme: use blocked chan?
	p.slotState = make([]*SlotState, state.ParallelExecNum)
