...
func (s *StateDB) AddLog(log *types.Log) {
	if s.isSlotDB {
		debug.Handler.LogWhenTracing("Slot=" + strconv.Itoa(s.SlotIndex) +
			" StateDB.AddLog thash:" + s.thash.String() + " len(Topics):" + strconv.Itoa(len(log.Topics)) +
			" log.Data:" + hex.EncodeToString(log.Data))
		for index, t := range log.Topics {
			debug.Handler.LogWhenTracing("Slot=" + strconv.Itoa(s.SlotIndex) +
				" topic " + strconv.Itoa(index) +
				" :" + t.String())
		}
	}
...
func (s *StateDB) Exist(addr common.Address) bool {
	debug.Handler.LogWhenTracing("Slot=" + strconv.Itoa(s.SlotIndex) +
		" StateDB.Exist addr:" + addr.String())
	if s.parallel.isSlotDB {
...
func (s *StateDB) SetStorage(addr common.Address, storage map[common.Hash]common.Hash) {
	...
	for key, value := range storage {
		debug.Handler.LogWhenTracing("Slot=" + strconv.Itoa(s.SlotIndex) + " key" + key.String() + " value:" + value.String())
	}
...
func (s *StateDB) Suicide(addr common.Address) bool {
	...
	if s.parallel.isSlotDB {
		...
		s.parallel.addrStateChangeInSlot[addr] = struct{}{} // address suicided.
	}
...
func (s *StateDB) TryPreload(block *types.Block, signer types.Signer) {
	accounts := make(map[common.Address]bool, block.Transactions().Len())  // better len*2, since from & to
	accountsSlice := make([]common.Address, 0, block.Transactions().Len()) // better len*2, since from & to
...
func (s *StateDB) PrepareAccessList(sender common.Address, dst *common.Address, precompiles []common.Address, list types.AccessList) {
	defer debug.Handler.StartRegionAuto("StateDB.PrepareAccessList")()
